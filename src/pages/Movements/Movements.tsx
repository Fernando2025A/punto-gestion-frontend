        ...
        <div className="movement-details-box">
          {Object.entries(movement.details).map(([key, value]) => (
            <div key={key} className="detail-item">
              <strong>{key === "imageUrl" ? "Imagen" : key}:</strong>{" "}
              {value?.from !== null && value?.from !== undefined  ? (
                <span style={{ display: key == "imageUrl" ? "none" : "flex"}} className="detail-from">{String(value.from)}</span>
              ) : null}
              {value?.from !== null && value?.from !== undefined ? " ➔ " : ""}
              <span className="detail-to">{String(key == "imageUrl" ? "Actualizada" : value?.to ?? "")}</span>
            </div>
          ))}
        </div>
      );
